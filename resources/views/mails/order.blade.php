...
<div>
    ...
    @if (WebpaymentsController::getPaymentsById($order->payment)[0]->isbank == 'Card' && $order->isadmin == 'No')
        <p><b>Можете да платите с Кредитна Карта ако все още не сте платили използвайки този линк:
            </b><a
                href="{{ route('user-order-email', ['id' => $order->order_id]) }}">{{ route('user-order-email', ['id' => $order->order_id]) }}</a>
        </p>
    @endif
    ...
    <table cellspacing="0" cellpadding="0" width="100%" border="1">
        ...
        @if (isset($order->items) and sizeof($order->items) > 0)
            @foreach ($order->items as $item)
                <tr>
                    <td>{{ ProductsController::getProductById($item['product_code'])->name }} x
        ...
        <tr>
            ...
            <td>{{ WebdeliveriesController::getDeliveriesById($order->delivery)[0]->name }}&nbsp;(Доставка
                +{{ number_format($order->price_delivery, 2, '.', '') }}&nbsp;{{ $dds_text }}</td>
        </tr>
        <tr>
...
Служител от фирма {{ FirmsController::getFirmById($order->firm_id)[0]->firm }} ще се свърже с Вас за уточнение на
поръчката. На телефон: {{ $order->phone }} или на email: {{ $order->email }}
