...
<script>
    ...
    function getValueByChecked(name) {
        return Number.parseFloat(document.querySelector('input[name="' + name + '"]:checked').getAttribute(
            "data-value"));
    }
</script>
...
@section('content')
    <input type="hidden" id="eik_if"
        value="<?php echo isset($currentUser) ? $currentUser->eik : ''; ?>" />
    <input type="hidden" id="firmname_if"
        value="<?php echo isset($currentUser) ? $currentUser->firmname : ''; ?>" />
    <input type="hidden" id="dds_nomer_if"
        value="<?php echo isset($currentUser) ? $currentUser->dds_nomer : ''; ?>" />
    <input type="hidden" id="firmcity_if"
        value="<?php echo isset($currentUser) ? $currentUser->firmcity : ''; ?>" />
    <input type="hidden" id="firmaddress_if"
        value="<?php echo isset($currentUser) ? $currentUser->firmaddress : ''; ?>" />
    <input type="hidden" id="mol_if"
        value="<?php echo isset($currentUser) ? $currentUser->mol : ''; ?>" />
    <h1><em>Поръчка</em></h1>
    ...
    <div class="container" id="order_div">
        @if (isset($cart_session) and sizeof($cart_session['firms']) > 0)
                    ...
                    <div class="card-body">
                        ...
                        @if (sizeof($firm['items']) > 0)
                            ...
                            @foreach ($firm['items'] as $item)
                                ...
                                <div class="row" style="padding-bottom:3px;">
                                    <div class="col-md-6">{{ $product_name }} <strong>x
                                            {{ $item['product_quantity'] }}</strong></div>
                                    <div class="col-md-6">{{ number_format($item['total_price'], 2, '.', '') }}
                                        {{ $item['product_currency'] }}</div>
                                </div>
                                <?php $total_price += floatval($item['total_price']); ?>
                                <?php $total_kg += floatval($item['product_real_kg']) *
                                floatval($item['product_quantity']); ?>
                                <?php $price_currency = $item['product_currency']; ?>
                            @endforeach

                            <div class="row">
                                <div class="col-md-6"><strong>Общо</strong></strong></div>
                                <div class="col-md-6"><strong>{{ number_format($total_price, 2, '.', '') }}
                                        {{ $price_currency }}</strong></div>
                            </div>
                            <input type="hidden" name="grand_total_price{{ $firm['firm_id'] }}"
                        ...
                        @if (sizeof($deliveries) > 0)
                            @foreach ($deliveries as $deliverie)
                                <?php
                                $subdeliveries = SubdeliveriesController::getSubdeliveriesByDelivery($deliverie->id,
                                $total_kg);
                                ...
                                @if ($transport_price != -1)
                                    <div class="row">
                                        ...
                                        <div class="form-check col-md-6">
                                            <input class="form-check-input" type="radio"
                                                onfocus="deliverieMethodsMouseUp(this.id, getValueByChecked(this.name));"
                                                data-value="{{ $transport_price }}"
                                                name="deliverieMethods{{ $firm['firm_id'] }}"
                                                id="deliverieMethods{{ $deliverie->id }}" value="{{ $deliverie->id }}" <?php if ($first_delivery==0) { echo 'checked' ; } else { if
                                                ($deliverie->isdefault == 'Yes') {
                                            echo 'checked';
                                            }
                                            } ?>>
                                    ...
                                    ?>
                                    <?php if ($deliverie->isdefault == 'Yes') {
                            ...
                            @endforeach
                        @else
                            <?php $error_methods_delivery = 'Липсва метод на доставка! Няма да можете да
                            закупите тази стока!'; ?>
                        ...
                        @if (sizeof($payments) > 0)
                            @foreach ($payments as $payment)
                                <div class="row">
                                    ...
                                    <div class="form-check col-md-6">
                                        <input class="form-check-input" type="radio"
                                            name="paymentMethods{{ $firm['firm_id'] }}"
                                            id="paymentMethods_{{ $payment->id }}" value="{{ $payment->id }}" <?php if ($first_payment==0) { echo 'checked' ; } else { if
                            ...
                            @endforeach
                        @else
                            <?php $error_methods_payment = 'Липсва метод на плащане! Няма да можете да
                            закупите тази стока!'; ?>
            ...
            <strong>Цена на стоките: <span
                    id="span_grand_total_price">{{ number_format($grand_total_price, 2, '.', '') }}</span>
                {{ $properties[0]->currency }}</strong><br />
            <strong>Цена за доставка на стоките: <span
                    id="span_grand_total_delivery">{{ number_format($grand_total_delivery, 2, '.', '') }}</span>
                {{ $properties[0]->currency }}</strong>
            <h4>Обща цена на поръчката: <span
                    id="span_grand_total_order">{{ number_format($grand_total_price + $grand_total_delivery, 2, '.', '') }}</span>
                {{ $properties[0]->currency }}</h4>
            <hr />
            <p>Вашите лични данни ще бъдат използвани за обработка на Вашата поръчка, както и за други цели, описани в
                нашата <a href="/politika" target="_blank" title="Политика за поверителност">Политика за поверителност</a>.
            </p>
            <div class="form-check">
                {{ Form::checkbox('saglasie', '', false, ['class' => 'form-check-input', 'required' => 'required']) }}
                <label class="form-check-label" for="saglasie">
                    Прочетох и се съгласявам с <a href="/terms" target="_blank" title="Общите условията на уебсайта">Общите
                        условията на уебсайта</a> *
                </label>
            </div>
